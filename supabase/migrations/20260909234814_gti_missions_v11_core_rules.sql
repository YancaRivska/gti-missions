...
begin
  ...
  select water_target_ml into v_target from public.gti_missions_profiles where user_id=v_uid and accepted_terms_at is not null and terms_version='1.1';
  if v_target is null then raise exception 'Current terms must be accepted'; end if;
  insert into public.gti_missions_water_days(user_id,day,target_ml,total_ml,log_count) values(v_uid,v_day,v_target,p_amount_ml,1)
  ...
  if v_completed_days>=7 then
    insert into public.gti_missions_weekly_badges(user_id,mission,week_start,label) values(v_uid,'aqua',v_week,'AquaXP 7/7 • Semana '||to_char(v_week,'IW')) on conflict(user_id,mission,week_start) do nothing returning id into v_badge_id;
...
begin
  ...
  select exercise_weekly_target into v_weekly_target from public.gti_missions_profiles where user_id=v_uid and accepted_terms_at is not null and terms_version='1.1';
...
declare
  ...
  v_total_xp bigint:=0; v_monthly_xp bigint:=0; v_badges integer:=0; v_custom_badges integer:=0; v_anchor date;
begin
  if v_uid is null or coalesce((auth.jwt()->>'is_anonymous')::boolean,true) then raise exception 'Permanent account required'; end if;
  select water_target_ml,exercise_weekly_target into v_target,v_exercise_target from public.gti_missions_profiles where user_id=v_uid and accepted_terms_at is not null and terms_version='1.1';
  ...
  select count(*) into v_custom_badges from public.gti_missions_challenge_badges where user_id=v_uid;
  return jsonb_build_object('water_today_ml',coalesce(v_today_ml,0),'water_target_ml',coalesce(v_target,2000),'water_streak',coalesce(v_streak,0),'water_week_days',coalesce(v_water_week_days,0),'water_week_target',7,'exercise_week_days',coalesce(v_exercise_days,0),'exercise_week_target',v_exercise_target,'total_xp',coalesce(v_total_xp,0),'monthly_xp',coalesce(v_monthly_xp,0),'badges',coalesce(v_badges,0)+coalesce(v_custom_badges,0));
end; $$;

drop function if exists public.gti_missions_leaderboard();
create function public.gti_missions_leaderboard()
...
language sql security definer set search_path=public,pg_temp as $$
  with monthly as (
    ...
    where m.xp>0 and p.accepted_terms_at is not null and p.terms_version='1.1'
  )
  ...
  where auth.uid() is not null and coalesce((auth.jwt()->>'is_anonymous')::boolean,true) is false
    and exists(select 1 from public.gti_missions_profiles me where me.user_id=auth.uid() and me.accepted_terms_at is not null and me.terms_version='1.1')
...
begin
  if v_uid is null or coalesce((auth.jwt()->>'is_anonymous')::boolean,true) then raise exception 'Permanent account required'; end if;
  delete from public.gti_missions_challenge_badges where user_id=v_uid;
  delete from public.gti_missions_challenge_entries where user_id=v_uid;
  delete from public.gti_missions_challenge_participants where user_id=v_uid;
  ...
  delete from public.gti_missions_exercise_days where user_id=v_uid;
  delete from public.gti_missions_exercise_weeks where user_id=v_uid;
  delete from public.gti_missions_admins where user_id=v_uid;
