create table if not exists public.gti_missions_exercise_days (
  user_id uuid not null references auth.users(id) on delete cascade,
  day date not null,
  total_minutes integer not null default 0 check (total_minutes between 0 and 300),
  log_count integer not null default 0 check (log_count between 0 and 10),
  primary key (user_id, day)
);
alter table public.gti_missions_exercise_days enable row level security;
drop policy if exists gti_missions_exercise_days_select_own on public.gti_missions_exercise_days;
create policy gti_missions_exercise_days_select_own on public.gti_missions_exercise_days for select to authenticated
...
revoke insert,update,delete on public.gti_missions_exercise_days from authenticated,anon;
...
begin
  ...
  if v_log_count<=5 then
    insert into public.gti_missions_xp_events(user_id,source,xp,event_key) values(v_uid,'water_log',5,'water_log:'||v_day::text||':'||v_log_count::text) on conflict do nothing;
    v_xp:=v_xp+5;
  end if;
  if v_total>=v_target then
    ...
    if coalesce(v_goal_new,false) then
      insert into public.gti_missions_xp_events(user_id,source,xp,event_key) values(v_uid,'water_goal',50,'water_goal:'||v_day::text)
  ...
  if v_completed_days>=5 then
    insert into public.gti_missions_weekly_badges(user_id,mission,week_start,label) values(v_uid,'aqua',v_week,'AquaXP • Semana '||to_char(v_week,'IW'))
    on conflict(user_id,mission,week_start) do nothing returning id into v_badge_id;
    if v_badge_id is not null then
      insert into public.gti_missions_xp_events(user_id,source,xp,event_key) values(v_uid,'weekly_badge',100,'badge:aqua:'||v_week::text) on conflict do nothing;
      v_xp:=v_xp+100;
    end if;
  end if;
