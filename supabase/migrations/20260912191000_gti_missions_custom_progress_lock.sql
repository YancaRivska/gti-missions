...
CREATE OR REPLACE FUNCTION public.gti_missions_log_custom_challenge(p_challenge_id uuid, p_value numeric DEFAULT 1, p_note text DEFAULT NULL::text, p_photo_path text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- Serializa atualizações da mesma missão diária para impedir perda de progresso concorrente.
  perform pg_advisory_xact_lock(hashtextextended(v_uid::text||':'||p_challenge_id::text||':'||v_today::text,0));
  ...
  if v_c.requires_photo and v_now_completed and not v_was_completed then
    if p_photo_path is null or p_photo_path='' then raise exception 'Camera proof required'; end if;
    select exists(
      select 1 from storage.objects o
      where o.bucket_id='gti-missions-checkins'
        and o.name=p_photo_path
        and (storage.foldername(o.name))[1]=v_uid::text
        and (storage.foldername(o.name))[2]=p_challenge_id::text
  ...
  insert into public.gti_missions_challenge_entries(
    challenge_id,user_id,entry_day,value,note,completed_at,photo_path,photo_expires_at
  ) values(
    p_challenge_id,v_uid,v_today,v_after,nullif(trim(coalesce(p_note,'')),''),
    case when v_now_completed then now() else null end,
    case when v_c.requires_photo and v_now_completed and not v_was_completed then p_photo_path else null end,
    case when v_c.requires_photo and v_now_completed and not v_was_completed then now()+interval '24 hours' else null end
  )
  on conflict(challenge_id,user_id,entry_day) do update set
    ...
    photo_path=coalesce(public.gti_missions_challenge_entries.photo_path,excluded.photo_path),
    photo_expires_at=coalesce(public.gti_missions_challenge_entries.photo_expires_at,excluded.photo_expires_at),
    updated_at=now();
  ...
  return jsonb_build_object(
    ...
    'xp_awarded',v_xp,
    'photo_expires_at',case when v_c.requires_photo and v_now_completed and not v_was_completed then now()+interval '24 hours' else null end
  );
end;
$function$

revoke all on function public.gti_missions_log_custom_challenge(uuid,numeric,text,text) from public,anon;
grant execute on function public.gti_missions_log_custom_challenge(uuid,numeric,text,text) to authenticated,service_role;
