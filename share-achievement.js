...
/** @param {{id:string, icon:string, name:string, description:string, activity?:boolean}} badge @param {{username:string}} player @param {string} season @param {number} height */
export async function shareAchievement(badge,player,season,height){
 ...
 line(badge.activity?'MISSÃO DO DIA':'CONQUISTA',height*.195,64,'#9eeaff');line(badge.activity?'REGISTRADA':'DESBLOQUEADA',height*.195+73,64,'#e7dbff');
 ...
 const words=badge.description.replace(/\s+/g,' ').trim().slice(0,600).split(' '),lines=[];let text='';const maxLines=height===1080?2:3;
 for(const word of words){if(ctx.measureText(text+' '+word).width>870&&text){lines.push(text);text=word;}else text+=(text?' ':'')+word;}if(text)lines.push(text);
 lines.slice(0,maxLines).forEach((content,index)=>{let value=content;if(index===maxLines-1&&lines.length>maxLines)value+='…';while(ctx.measureText(value).width>870&&value.length>1)value=value.slice(0,-2)+'…';ctx.fillText(value,540,height*.735+index*40);});
 line('@'+player.username,height*.85,38,'#8ceaff');line(season,height*.905,28,'#b7c8e6');
 line('ACESSE O APP · GALERA DO TI',height*.95,23,'#8ea9c9');line('gti-missions-yanca-rivska.vercel.app',height*.977,25,'#9eeaff');
 /** @type {Blob|null} */
...
/** @param {{title:string, detail:string, icon:string, xp:number, date:string}} activity @param {{username:string}} player @param {number} height */
export async function shareActivity(activity,player,height=1920){
 return shareAchievement({id:'atividade',name:activity.title,description:activity.detail,icon:activity.icon,activity:true},player,`${activity.date} · +${Number(activity.xp)||0} XP`,height);
}
