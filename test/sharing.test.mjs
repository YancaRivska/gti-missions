...
async function share(height,available,fail=false){
 const calls={downloads:0,shares:0};const drawing={createLinearGradient:()=>({addColorStop(){}}),fillRect(){},fillText(){},measureText:t=>({width:t.length*16})};
 const canvas={getContext:()=>drawing,toBlob:callback=>callback(new Blob(['image'],{type:'image/png'}))};
 const context=vm.createContext({Blob,File,URL,Error,setTimeout:callback=>callback(),document:{createElement:tag=>tag==='canvas'?canvas:{click(){calls.downloads++;}}},navigator:{canShare:()=>available,share:async()=>{calls.shares++;if(fail)throw new Error('Unavailable');}}});
